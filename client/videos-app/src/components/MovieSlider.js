...
const MovieSlider = ({ index = 0, movies, displayCount = 4 }) => {
    ...
    if (Array.isArray(displayedMovies)) {
        if (displayedMovies.length < displayCount) {
            const emptyMovies = Array(displayCount - displayedMovies.length).fill({});
            displayedMovies.push(...emptyMovies);
        }
    }
